...
public class AdminController {
    ...
    @GetMapping("/comercial/{id}/stats-full")
    ...
    public ResponseEntity<?> reasignarCliente(@RequestBody Map<String, String> body) {
        try {
            // First deactivate old assignment
            // Then assign new
            // This logic is simplified in service
            Asignacion asignacion = adminService.asignarCliente(
                    body.get("clienteId"),
                    body.get("nuevoComercialId"),
                    body.get("adminId"));
            return ResponseEntity.ok(asignacion);
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(Map.of("message", e.getMessage()));
